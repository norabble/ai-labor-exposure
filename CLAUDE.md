...
**`seeds/cps_occupation_panel.csv` follows the same convention, with one difference.** `cps_historical_panel.py`'s `merge_into_seed` merges a fresh BLS API fetch for the ten CPS occupation-group series into this committed panel the same way — a run that cannot reach BLS still renders from the seed alone. Unlike `cps_a19_panel.csv`, there is no scripted fetch for it yet: it is not part of `make download-data`, has no `main.py` stage, and `release.yml` neither refreshes nor promotes it. The seed is currently updated only by invoking `cps_historical_panel.py`'s functions directly and committing the result by hand, the same way `data/output/cps_occupation_panel.csv` and `cps_group_trends.csv` were produced for this instrument's initial 1983–2026 build.
