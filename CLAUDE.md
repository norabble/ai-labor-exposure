...
| `historical_displacement_rate.csv` | `historical_displacement.py` | Economy-wide annual displacement rate D — the demand composition model's only empirical input, replacing AI penetration. Long form over five sources (`dws_all_tenures`, `dws_long_tenured`, `dws_structural_all_tenures`, `dws_structural_long_tenured`, `productivity`) so the model can be swept across them. `is_interpolated` marks DWS rows, where one survey-window average is repeated across the years it covers. D cancels out of every cross-sectional correlation, so it governs the model's amplitude and time variation, not its shape. |
