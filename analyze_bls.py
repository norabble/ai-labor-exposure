...
def _numeric_bls_columns(bls_dataframe: pd.DataFrame) -> pd.DataFrame:
    """Coerce TOT_EMP / A_MEDIAN / H_MEDIAN, which older files store as strings with commas and '*' suppressions."""
...
def attach_growth_columns(trend_df: pd.DataFrame, available_years: list[str]) -> pd.DataFrame:
    ...
    column layouts. Wage growth falls back to H_MEDIAN where the annual series
    is unavailable — see compute_wage_growth.
    """
    ...
    for prev_year, curr_year in zip(available_years[:-1], available_years[1:]):
        ...
        if f"A_MEDIAN_{prev_year}" in trend_df.columns and f"A_MEDIAN_{curr_year}" in trend_df.columns:
            wage_growth, wage_growth_source = compute_wage_growth(trend_df, prev_year, curr_year)
            trend_df[f"{prefix}wage_growth_{prev_year}_{curr_year}"] = wage_growth
    ...
    if later_years:
        ...
        if f"A_MEDIAN_{latest_year}" in trend_df.columns and f"A_MEDIAN_{COMPOSITE_ANCHOR_YEAR}" in trend_df.columns:
            composite_wage_growth, composite_wage_source = compute_wage_growth(trend_df, COMPOSITE_ANCHOR_YEAR, latest_year)
            trend_df["wage_growth_composite"] = composite_wage_growth
            trend_df["wage_source_composite"] = composite_wage_source
    ...
    for year_suffix, zip_path in YEAR_CONFIGS:
        if os.path.exists(zip_path):
            ...
            if year_frame is not None:
                detailed_rows_df = select_detailed_rows(year_frame)
                report_wage_suppression(year_suffix, detailed_rows_df)
                    ...
                    censoring_rows.append(
                        {
                            ...
                            "n_censored": int(detailed_rows_df["A_MEDIAN_censored"].sum()),
                            "n_withheld": int(detailed_rows_df["A_MEDIAN_withheld"].sum()),
                            "n_detailed": len(detailed_rows_df),
                        }
                    )
                year_dataframes[year_suffix] = detailed_rows_df
