...
if not GCP_PROJECT_ID:
    raise ValueError(
        "GCP_PROJECT_ID environment variable is not set. Please set it in .env before running this script."
    )
...
def classify_all(delay_between_tasks: float = 0.3):
    ...
    if os.path.exists(CHECKPOINT_PATH):
        ...
        done_ids = set(df_ckpt["Task ID"].astype(str).tolist())
        existing_rows = df_ckpt.to_dict(orient="records")
        print(f"Resuming from checkpoint — {len(done_ids)} tasks already done.")

    # ── Iterate grouped by O*NET-SOC Code ────────────────────────────────────
    groups = df_tasks.groupby("O*NET-SOC Code", sort=True)
    ...
    for occ_code, group in groups:
        ...
        for _, row in group.iterrows():
            ...
            try:
                res_dict = classify_task(occ_title, task_statement)
                results.append({
