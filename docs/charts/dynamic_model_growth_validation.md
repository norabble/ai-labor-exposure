...
The sector-adjusted null does not mean the model is wrong about the direction of long-run labor reallocation. It means the model's current absorption mechanism is too coarse to produce occupation-level predictions that outperform the sector trend alone. The sector-level validation (`dynamic_sector_level_validation.md`) shows the model does have real predictive content at the sector level (r = +0.528, p = 0.012 for employment), which the occupation-level charts here are diluting.
