...
class TestComputeTaskImpact:
    def test_bounded_reduces_demand(self):
        result = compute_task_impact(_task_df([(0.5, "Bounded", 1.0)]))
        assert result["task_net_impact"].iloc[0] == pytest.approx(-0.5 * BOUNDED_DECLINE)

    def test_unbounded_expands_demand(self):
        result = compute_task_impact(_task_df([(0.5, "Unbounded", 1.0)]))
        assert result["task_net_impact"].iloc[0] == pytest.approx(0.5 * UNBOUNDED_REBOUND)

    def test_adversarial_rebounds_fully(self):
        result = compute_task_impact(_task_df([(0.5, "Adversarial", 1.0)]))
        assert result["task_net_impact"].iloc[0] == pytest.approx(0.5 * ADVERSARIAL_REBOUND)

    def test_zero_penetration_contributes_nothing(self):
        result = compute_task_impact(_task_df([(0.0, "Bounded", 1.0)]))
        assert result["task_net_impact"].iloc[0] == 0.0
    ...
    def test_importance_scales_impact(self):
        ...
        assert result["task_net_impact"].iloc[0] == pytest.approx(-0.5 * BOUNDED_DECLINE * 3.0)
...
class TestDeriveNarrative:
    ...
    def test_strong_decline(self):
        assert derive_narrative(self._row(-0.05, 0.10)) == "Strong Decline (Efficiency Transition)"

    def test_mild_decline(self):
        assert derive_narrative(self._row(-0.01, 0.05)) == "Mild Decline"

    def test_arms_race(self):
        assert derive_narrative(self._row(0.06, 0.01)) == "The Arms Race (Stable / Inflation)"

    def test_infinite_frontier(self):
        assert derive_narrative(self._row(0.02, 0.01)) == "The Infinite Frontier (Expansion)"

    def test_limited_impact_fallback(self):
        assert derive_narrative(self._row(-0.001, 0.001)) == "Limited AI Impact"

    def test_strong_decline_requires_sufficient_penetration(self):
        # Score qualifies but penetration is too low — should not be Strong Decline
        result = derive_narrative(self._row(-0.05, 0.01))
        assert result != "Strong Decline (Efficiency Transition)"
...
class TestRollupToOccupation:
    def _occ_df(self, rows):
        return pd.DataFrame(
            rows,
            columns=["O*NET-SOC Code", "Title", "Demand Type", "penetration", "task_importance", "task_net_impact"],
        )
    ...
    def test_pct_columns_sum_to_one(self):
        df = self._occ_df(
            [
                ...
                ("11-1011.00", "Chief Executives", "Adversarial", 0.2, 1.0, 0.2),
            ]
        )
        result = rollup_to_occupation(df)
    ...
    def test_occupation_impact_is_importance_weighted(self):
        # Two tasks: Bounded with importance 2 and Unbounded with importance 1
        # task_net_impact: -1.0 and +0.15
        # Expected: (-1.0 + 0.15) / (2.0 + 1.0) = -0.85 / 3.0
        df = self._occ_df(
            [
                ("11-1011.00", "Chief Executives", "Bounded", 0.5, 2.0, -1.0),
                ("11-1011.00", "Chief Executives", "Unbounded", 0.3, 1.0, 0.15),
        ...
        result = rollup_to_occupation(df)
        assert result.iloc[0]["occupation_impact"] == pytest.approx(-0.85 / 3.0)
